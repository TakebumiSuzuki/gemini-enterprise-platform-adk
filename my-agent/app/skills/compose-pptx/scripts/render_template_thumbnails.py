...
Usage:
    python render_template_thumbnails.py template.pptx [output_prefix] [--cols N]

Examples:
    python render_template_thumbnails.py template.pptx
    # -> template_thumbnails.jpg  (or -1.jpg, -2.jpg for large decks)

    python render_template_thumbnails.py template.pptx grid --cols 4
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Render template .pptx slides into labelled thumbnail grids."
    )
    parser.add_argument("input", help="Input PowerPoint template (.pptx)")
    parser.add_argument(
        "output_prefix",
        nargs="?",
        default="template_thumbnails",
        help="Output prefix for image files (default: template_thumbnails)",
    )
    parser.add_argument(
        "--cols",
        type=int,
...
def get_slide_info(pptx_path: Path) -> list[dict]:
    """Return one dict per slide, in presentation (sldIdLst) order."""
...
def build_slide_list(
    ...
    for pres_idx, info in enumerate(slide_info):
        # Label by 0-based presentation index so it matches inspect_template.py
        # and mapping.json's template_index.
        if info["hidden"]:
            placeholder_path = temp_dir / f"hidden-{info['name']}.jpg"
            placeholder_img = create_hidden_placeholder(placeholder_size)
            placeholder_img.save(placeholder_path, "JPEG")
            slides.append((placeholder_path, f"idx: {pres_idx} (hidden)"))
        else:
            if visible_idx < len(visible_images):
                slides.append((visible_images[visible_idx], f"idx: {pres_idx}"))
                visible_idx += 1
